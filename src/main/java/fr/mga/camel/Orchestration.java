...
public class Orchestration {
    ...
    public RouteBuilder directToActiveMQ() {
        return new RouteBuilder() {
            @Override public void configure() throws Exception {
                from("direct:start")
                        .to("activemq:queue:confirm-booking?mapJmsMessage=true&requestTimeout=10000&transferExchange=false&replyTo=confirm-booking-reply&concurrentConsumers=5&requestTimeoutCheckerInterval=500");//&transactedInOut=true&replyToType=Exclusive");
            }
        };
    }
    ...
    public RouteBuilder activeMQToCode(final LoggerProcessor activeMQProcessor) {
        return new RouteBuilder() {
            @Override public void configure() throws Exception {
                from("activemq:queue:confirm-booking?mapJmsMessage=true&requestTimeout=10000&transferExchange=false&replyTo=confirm-booking-reply&concurrentConsumers=5&requestTimeoutCheckerInterval=500")//&transactedInOut=true&replyToType=Exclusive")
